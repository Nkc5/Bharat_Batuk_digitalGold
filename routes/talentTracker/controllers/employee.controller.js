...
async function companyLogin(req, res) {
  ...
  if (data) {
    ...
    console.log("companyName sever login", data2.name)
    return res.json({ message: "Login Successfully", "companyName": data2.name })
...
async function companyLogout(req, res) {
  ...
  var userLogin = loginModel.findOne({ sessionID })

  console.log("sessionID in logout", sessionID)

  if (!userLogin) {
    return res.json("Already Logout, Please Login")
...
function dummyExcel(req, res) {

  const viewsDir = path.join(__dirname, "../", "public", "downloads");

  console.log("viewsDir", viewsDir)

  const filePath = path.join(viewsDir, "Employee.csv");
